...
is monotone, so ranking is preserved exactly while the scale is corrected.
Measured, it takes the distribution shift from 0.92 to 0.09 — and the report
still warns you if your pipeline uses a fixed threshold, because the right fix
is to retune it.
