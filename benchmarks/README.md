...
Micro-benchmarks that run on every PR under CodSpeed's instruction-count
instrumentation, plus the harnesses used to produce the measurements in
`docs/`.
...
| | Where | Gate |
|---|---|---|
| `tests/performance/test_hot_path.py` | every PR, CodSpeed | 5% warn, 15% block |
...
exactly the regressions worth catching. Only the instruction-count and
absolute-memory measurements are gates.
